...
# Load Data Set
datos = read_csv("datos_final.csv")
...
datos_long <- datos %>%
  ...
  summarise(
    ID_check = ID_check,
    grupo = grupo,
    zeros_SELF = zeros_SELF,
    zeros_OTHER = zeros_OTHER,
    trials = list(1:48),
    .groups = 'drop'
  ) %>%
  unnest(trials) %>%
  ...
  select(sub, trial, decision, reward, effort, agent, success, grupo = grupo_num, zeros_SELF, zeros_OTHER) %>%
  
  # Order
  arrange(sub, trial)

# Save Data long
write.csv(datos_long, "datos_long_models.csv", row.names = FALSE)


## FILTRAR POR PARÁMETROS
datos_long <- datos_long[datos_long$zeros_OTHER <= 25, ] 
datos_long <- datos_long[datos_long$zeros_SELF <=25, ]
datos_long = datos_long[datos_long$sub != 4436, ]
